...
from utils.yolo_classes import load_classes
...
class yolov4(object):
    def __init__(self, model, input_shape, category_num=80, cuda_ctx=None):
        ...
        self.trt_yolo = TrtYOLO((self.model_path + self.model), (self.h, self.w), self.category_num)

        #self.trt_logger = trt.Logger(trt.Logger.INFO)
        #self.engine = self._load_engine()
        #self.context = self.engine.create_execution_context()
        #grid_sizes = get_yolo_grid_sizes(self.model, self.input_shape[0], self.input_shape[1])
        #self.inputs, self.outputs, self.bindings, self.stream = self.allocate_buffers(self.engine, grid_sizes)

        worker_thread = threading.Thread(target=self.cuda_callback())
        worker_thread = threading.start()
        worker_thread = threading.join()

    def cuda_callback(self):
        """Workaround for pycuda.autoinit."""
        cuda.init()
    ...
    def init_params(self):
        ...
        #self.video_topic1 = rospy.get_param("/video_topic1", "/zed/zed_node/left_raw/image_raw_color")
        self.video_topic1 = rospy.get_param("/video_topic1", "video_source/raw")
        self.video_topic2 = rospy.get_param("/video_topic2", "/zed/zed_node/left_raw/image_raw_color")
        self.video_topic3 = rospy.get_param("/video_topic3", "/zed/zed_node/right_raw/image_raw_color")
        self.video_topic4 = rospy.get_param("/video_topic4", "/zed/zed_node/right_raw/image_raw_color")
        ...
        self.category_num = rospy.get_param("/category_number", 9)
        self.names = rospy.get_param("/names", package_path + "/jaws.names")
        ...
        self.image_sub1 = rospy.Subscriber(self.video_topic1, Image, self.img_callback1, queue_size=1, buff_size=2**26)
        self.image_sub2 = rospy.Subscriber(self.video_topic2, Image, self.img_callback2, queue_size=1, buff_size=2**26)
        self.image_sub3 = rospy.Subscriber(self.video_topic3, Image, self.img_callback3, queue_size=1, buff_size=2**26)
        self.image_sub4 = rospy.Subscriber(self.video_topic4, Image, self.img_callback4, queue_size=1, buff_size=2**26)
        self.detection_pub = rospy.Publisher("detections", Detector2DArray, queue_size=1)
        self.overlay_pub = rospy.Publisher("/result/overlay", Image, queue_size=1)
    ...
    def init_yolo(self):
        ...
        cls_dict = load_classes(self.names)
    ...
    def img_callback1(self, ros_img):
        ...
        try:
            overlay_img = self.bridge.cv2_to_imgmsg(cv_img, encoding="passthrough")
