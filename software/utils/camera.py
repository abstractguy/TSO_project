...
import cv2, logging, numpy as np, subprocess, threading
...
def open_cam_rtsp(uri, width, height, latency):
    ...
    if 'omxh264dec' in gst_elements:
        ...
        # NOTE: in case resizing images is necessary, try adding a 'videoscale' into the pipeline.
...
def open_cam_rtp(port=1337):
    # On sender side:
    #     ffmpeg -re -i doc/15062016_16360868.6474.m2t -c copy -f rtp udp://192.168.55.1:1337
    #gst_str = '-v udpsrc port={} caps="application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96" ! rtph264depay ! decodebin ! videoconvert ! autovideosink'.format(port)
    gst_elements = str(subprocess.check_output('gst-inspect-1.0'))
    if 'omxh264dec' in gst_elements:
        #gst_str = 'udpsrc port={} fifo_size=50000 overrun_nonfatal caps="application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96" ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! appsink'.format(port)
        #gst_str = 'udpsrc port={} caps="application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96" ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! appsink'.format(port)
        gst_str = 'udpsrc port={} caps="application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96" ! rtph264depay ! decodebin ! videoconvert ! autovideosink'.format(port)
    else:
        raise RuntimeError('H.264 decoder not found!')
    return cv2.VideoCapture(gst_str, cv2.CAP_GSTREAMER)
...
def open_cam_onboard(width, height):
    ...
    if 'nvcamerasrc' in gst_elements:
        # On versions of L4T prior to 28.1, you might need to add 'flip-method=2' into gst_str below.
        ...
        gst_str = ('nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080, format=(string)NV12, framerate=(fraction)30/1 ! nvvidconv flip-method=2 ! video/x-raw, width=(int){}, height=(int){}, format=(string)BGRx ! videoconvert ! appsink').format(width, height)
    else:
...
class Camera():
    ...
    def _open(self):
        ...
        #ffmpeg -stream_loop -1 -re -i doc/15062016_16360868.6474.m2t -c copy -vcodec libx264 -f mpegts udp://192.168.55.1:1337
            ...
            logging.info('Camera: using raw UDP stream at port %s' % a.udp_port)
            self.cap = cv2.VideoCapture('udp://@:%s?overrun_nonfatal=1&fifo_size=50000000' % a.udp_port, cv2.CAP_FFMPEG)
            self._start()
        elif a.video:
            ...
            logging.info('Camera: using RTP stream %s' % a.rtp)
            self.cap = open_cam_rtp(a.udp_port)
            self._start()
    ...
    def _start(self):
        ...
        if self.video_file:
            ...
            self.img_height, self.img_width, _ = self.img_handle.shape
            # Start the child thread if not using a video file source i.e. RTSP, USB or onboard.
