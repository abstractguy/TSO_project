...
# File:      software/jetson/utils/convert_DarkNet_to_ONNX.py
# By:        Samuel Duclos
# For:       Myself
# Reference: https://github.com/jkjung-avt/tensorrt_demos.git
# Usage:     #python3 converters/convert_DarkNet_to_ONNX.py --darknet-weights ./fastmot/models/yolov4.weights --onnx-weights ./fastmot/models/yolov4.onnx --cfg ./utils/cfg/yolov4.cfg --image-shape 608 608 --names ./utils/cfg/coco.names --batch-size 1 --dla_core 0 --int8 --add-plugins
#            python3 converters/convert_DarkNet_to_ONNX.py --darknet-weights ./fastmot/models/yolov4.weights --onnx-weights ./fastmot/models/yolov4.onnx --cfg ./utils/cfg/yolov4.cfg --image-shape 608 608 --names ./utils/cfg/coco.names --batch-size 1 --add-plugins

from __future__ import print_function
from collections import OrderedDict

import argparse, hashlib, numpy as np, onnx, os, sys
...
def parse_args():
    ...
    parser.add_argument('--darknet-weights', metavar='<darknet-weights>', type=str, default='./weights/yolov4.weights', help='Path to DarkNet weights.')
    parser.add_argument('--onnx-weights', metavar='<onnx-weights>', type=str, default='./weights/yolov4.onnx', help='Path to ONNX-converted weights.')
    parser.add_argument('--cfg', metavar='<cfg>', type=str, required=False, default='./utils/cfg/yolov4.cfg', help='DarkNet *.cfg file.')
    parser.add_argument('--names', metavar='<names>', type=str, required=False, default='./cfg/coco.names', help='DarkNet *.names file.')
    parser.add_argument('--batch-size', metavar='<batch-size>', type=int, default=1, help='Batch size.')
    parser.add_argument('--image-shape', metavar='<image-shape>', nargs='+', type=int, default=[640, 640], help='Size of image.')
    parser.add_argument('--dla-core', metavar='<dla-core>', type=int, default=-1, help='ID of DLA core for inference (0 ~ N-1).')
    parser.add_argument('--fp32', action='store_true', help='Floating point, 32-bit calculations (default is 16-bit).')
    parser.add_argument('--int8', action='store_true', help='Build INT8 TensorRT engine.')
    parser.add_argument('--add-plugins', action='store_true', help='Add Yolo plugins.')
    parser.add_argument('--no-verbose', action='store_true', help='Don\'t dump verbose ONNX conversion logs on screen.')
...
def verify_classes(cfg, num_classes):
...
def main():
    args = parse_args()
    model_name = os.path.splitext(os.path.basename(args.darknet_weights))[0]
    category_num = len(load_classes(args.names))
    w, h = args.image_shape

    if not os.path.isfile(args.cfg):
        raise SystemExit('ERROR: file (%s) not found!' % args.cfg)
    if not os.path.isfile(args.darknet_weights):
        raise SystemExit('ERROR: file (%s) not found!' % args.darknet_weights)
    if not verify_classes(args.cfg, category_num):
        raise SystemExit('ERROR: bad names file contains %d categories' % category_num)
    ...
    supported_layers = ['net', 'convolutional', 'maxpool', 'shortcut', 'route', 'upsample', 'yolo']
    ...
    layer_configs = parser.parse_cfg_file(args.cfg)
    ...
    if 'yolov3' in model_name:
        ...
        raise SystemExit('ERROR: unknown model (%s)!' % args.darknet_weights)

    # Create a GraphBuilderONNX object with the specified output tensor dimensions.
    print('Building ONNX graph...')
    builder = GraphBuilderONNX(model_name, output_tensor_dims)

    # Now generate an ONNX graph with weights from the previously parsed layer configurations and the weights file.
    yolo_model_def = builder.build_onnx_graph(layer_configs=layer_configs, weights_file_path=args.darknet_weights, verbose=not args.no_verbose)
    ...
    onnx.save(yolo_model_def, args.onnx_weights)
