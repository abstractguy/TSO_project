...
# File:        software/jetson/utils/camera2.py
# By:          Samuel Duclos
# For:         Myself
# Description: This file implements a camera protocol. REWRITE ME!!
...
import logging, numpy as np, picamera, time
    ...
    while not capture_manager.stopped:
        if capture_manager.frame is not None:
            ...
            if any(item in label_idxs for item in prediction.get('detection_classes')):
                ...
                tracked = ((i, x) for i, x in enumerate(prediction.get('detection_classes')) if x in label_idxs)
...
class PiCameraStream(object):
    ...
    def __init__(self,
        ...
        self.stream = self.camera.capture_continuous(self.data_container, format="rgb", use_video_port=True)
    ...
    def flush(self):
        """Loop until self.stopped flag is flipped, grab the first frame in buffer, then empty buffer."""
